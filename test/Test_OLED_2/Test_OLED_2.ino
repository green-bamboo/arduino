...
#define SSID "TestData"                              // insert your SSID
...
void setup(void) {
	...
	Wire.begin(1, 3);                                // Initialize I2C and OLED Display
	init_OLED();                                    // 
	reset_display();
	...
	sendStrXY("SSID :", 0, 0);
	//sendStrXY(SSID, 7, 0); // prints SSID on OLED

	sendStrXY("192.168.0.255", 0, 1);
	...
	sendStrXY("WebServer ready!", 4, 2);

	//sendStrXY("Line 20 maybe", 6, 20);

	//sendStrXY("Line 30 maybe", 6, 30);

	//sendStrXY("Line 40 maybe", 6, 40);
